...
def overlay_boxes(image, predictions, color):
    ...
    for box in boxes:
        ...
        image = cv2.rectangle(image, tuple(top_left), tuple(bottom_right), color, 3)

    return image


def create_vizualization_grid(
    images: torch.Tensor,
    ...
    """
    Creates visualizations for MaskRCNN/FasterRCNN outputs

    :param images: tensors for input images
    :param outputs: dictionary with predicted boxes and masks
    :param targets: dictionary with groundtruth boxes and masks
    :param n: number of images to include in grid

    :return: numpy array with images overlayed with outputs and GT boxes
    """
    # Get n images at random from batch
    p = torch.tensor([1 / len(images)] * len(images))
    idcs = p.multinomial(min(n, len(images)))

    # Subset by sampled indices
    ...
    # Denormalize images and overlay GT and predicted boxes
    images_denorm = (
        inv_normalize(torch.vstack(images)).squeeze(0).detach().cpu().numpy()
    )
    images_denorm = [cv2.cvtColor(img, cv2.COLOR_GRAY2RGB) for img in images_denorm]
    images_denorm = [
        overlay_boxes(ele, outputs[idx], color=(150, 0, 0))
        for idx, ele in enumerate(images_denorm)
    ]
    images_denorm = [
        overlay_boxes(ele, targets[idx], color=(0, 150, 0))
        for idx, ele in enumerate(images_denorm)
    ]

    # Create grid and populate with boxes
