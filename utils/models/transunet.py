...
class TransUnet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.vit = (
            ...
            if vit_transformer is None
            else vit_transformer
        )

        # initiate classification head
        self.classification_head = ClassificationHead(
            in_channels=512, classes=classes, dropout=dropout_regression
        )
    ...
    def forward(self, x):
        ...
        y = rearrange(
            ...
            x=self.img_dim_vit // self.patch_size,
            y=self.img_dim_vit // self.patch_size,
            patch_x=self.patch_size,
            patch_y=self.patch_size,
        )

        y = self.vit_conv(y)
        count = self.classification_head(y)
        y = self.dec1(y, x8)
        y = self.dec2(y, x4)
        y = self.dec3(y, x2)
        y = self.dec4(y)
